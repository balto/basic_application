...
class ENestedSetBehavior extends CActiveRecordBehavior
{
	...
	public function delete()
	{
		...
		if($owner->getIsNewRecord())
			throw new CDbException('The node cannot be deleted because it is new.');
	...
	public function prependTo($target,$runValidation=true,$attributes=null)
	{
		...
		if(!$owner->getIsNewRecord())
			throw new CDbException('The node cannot be inserted because it is not new.');
	...
	public function insertBefore($target,$runValidation=true,$attributes=null)
	{
		...
		if($target->isRoot())
			throw new CException('The target node should not be root.');
	...
	public function moveBefore($target)
	{
		...
		if($owner->getIsNewRecord())
			throw new CException('The node should not be new record.');

		if($owner->equals($target))
			throw new CException('The target node should not be self.');

		if($target->isDescendantOf($owner))
			throw new CException('The target node should not be descendant.');
	...
	public function beforeSave($event)
	{
		if($this->_ignoreEvent)
			...
			throw new CDbException('You should not use CActiveRecord::save() method when ENestedSetBehavior attached.');
	}
	...
	public function beforeDelete($event)
	{
		if($this->_ignoreEvent)
			...
			throw new CDbException('You should not use CActiveRecord::delete() method when ENestedSetBehavior attached.');
	}
	...
	{
		if(!$this->hasManyRoots)
			throw new CException('Many roots mode is off.');
